...
boxplot(rm, col = "mistyrose", border="darkred", main="krabicak")
boxplot(rm, col = "mistyrose", border="darkred", main="krabicak",range=3)
mean(rm)
median(rm)
mean(rm,trim=0.1)
sum(rm<=4.5)/length(rm)
HuberM(rm)
...
nox <- Boston$nox
hist(nox,col="skyblue",border="darkblue",freq=F,main="Histogram",xlab="oxidy ",ylab="Hustota", breaks = 30)
...
plot(as.factor(prom2)~as.factor(prom2),col=2:7)
...
prom7<-Boston.kat$chas
prom8<-Boston.kat$tax.k
(tab<-table(prom7,prom8))

round(addmargins(prop.table(tab)),3)
round(addmargins(prop.table(tab,1)),3)
round(addmargins(prop.table(tab,2)),3)

plot(as.factor(prom7)~as.factor(prom8),col=2:7)
plot(as.factor(prom8)~as.factor(prom7),col=2:7)
#zavisí výskyt bakterii na léčbě?
data(bacteria)
bac<- bacteria$y
trt<- bacteria$trt
...
(tab<-table(bac,trt))
roud(addmargins(prop.table(tab,1)),3)
...
# a co korelace
#cor patří do intervalu (-1,1)
#-1 ... ideální nempřímá (jedna klesá, druhá stoupá)(hodnoty leží na klesajíxí přímce)
# 0 ... lineární nezávislost (nekorelovast)
# 1 ... ideální přímá (hodnoty leží na stoupající přímce) silna > 0,7
...
ciselne <- data.frame(Boston$crim, Boston$nox, Boston$rm, Boston$lstat, Boston$medv)
cor(ciselne)
pairs(ciselne)
#############################

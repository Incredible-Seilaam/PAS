...
## Na 5% hladine vyznamnosti rozhodnete, zda stredni hodnota porodni hmotnosti deti muze byt 3.5kg.
  porhmt <- Kojeni$porHmotnost
  MeanCI(porhmt, conf.level = 0.95)
  t.test(porhmt, mu= 3500)
  #Pravděpodobně ano, hodnota je v intervalu je je blízko mean = 3469,576
  
## V jakem rozmezi by se s 90% spolehlivosti mela nachazet stredni hodnota porodni delky deti?
  pordlk <- Kojeni$porDelka
  MeanCI(pordlk, conf.level = 0.90)
  #Střední hodnota by se měla nacházet v intervalu mezi (50.33706 ; 50.85486)
...
BinomCI(table(otec)[2], sum(table(otec)), method = "wald")
    #pocet priz. možn.(přítomen), soucet 2 hodnot, metoda
...
koj <- Kojeni$Koj24
table(koj)
round(prop.table(table(koj)), 4) * 100

BinomCI(table(koj)[1], sum(table(koj)), conf.level=0.9, method = "wald")*100
...
# vypocet 
BinomDiffCI(x1 = tab[2,1], n1 = tab[1,1] + tab[2,1], x2 = tab[2,2], n2 = tab[1,2] + tab[2,2], method ="wald")*100
#x1 = počet příznivých možností (17 praha), n1 = počet všech možností (všech v praze 70) - první populace
#x2 = počet příznivých možností (19 ores), n2 = počet všech možností (všech v okres 29) - druhá populace
...
BinomDiffCI(x1 = tab[1,1], n1 = tab[1,1] + tab[2,1], x2 = tab[1,2], n2 = tab[1,2] + tab[2,2],conf.level = 0.99, method ="wald")*100

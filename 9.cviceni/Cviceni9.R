...
## Jake rozdeleni ma cholesterol? 
systlak <- Stulong$syst1
hist(systlak,col="azure", border="darkblue", main="Histogram pro systolicky tlak", xlab="systolicky tlak")
PlotQQ(systlak, pch=19)
Skew(systlak) #sikmost
Kurt(systlak) #spicatost

descdist(systlak ,discrete=FALSE, boot=1000)

(fit1 <- fitdist(systlak,"lnorm"))
(fit2 <- fitdist(systlak,"gamma"))
(fit3 <- fitdist(systlak,"weibull"))

data.frame(distr=c("lnorm","gamma","weibull"),
           AIC=c(fit1$aic,fit2$aic,fit3$aic),
           BIC=c(fit1$bic,fit2$bic,fit3$bic)
)
...
PlotQQ(systlak, qdist=function(p) qlnorm(p,meanlog=coef(fit1)[1],sdlog=coef(fit1)[2]))
PlotQQ(systlak, qdist=function(p) qgamma(p,shape=coef(fit2)[1],rate=coef(fit2)[2]))
PlotQQ(systlak, qdist=function(p) qweibull(p,shape=coef(fit3)[1],scale=coef(fit3)[2]))
...
(fit3 <- fitdist(cukr,"exp"))

data.frame(distr=c("norm","logis","exp"),
           AIC=c(fit1$aic,fit2$aic,fit3$aic),
           BIC=c(fit1$bic,fit2$bic,fit3$bic)
)
...
PlotQQ(cukr, qdist=function(p) qnorm(p,mean=coef(fit1)[1],sd=coef(fit1)[2]))
...
# x_ ... nejlepší bodový odhad střední hodnoty
...
print("Střední hodnota pro 'hp' je v intervalu mezi *113.43* a *179.95*")
...
tapply(qsec, vs, mean)
  ...
  # druhy zpusob je pres dvouvyberovy t-test pro ruzne rozptyly
  t.test(qsec ~ vs)
## Lisi se od sebe vaha vozu (promenna wt) podle typu prevodovky?
